...
img_blur_kernel(const float *d_img_input, float *d_img_output, int kernel_size, int img_height, int img_width) {
    ...
    if (r >= img_height || c >= img_width || kernel_size % 2 == 0 || kernel_size <= 0) // Avoid out of image borders and even kernel size
    {
        return;
    }
    ...
    int num_pixels = 1;
    float pixels_sum = d_img_input[i_thread];
...
void Kernel::run(const float *d_img_input, float *d_img_output, int kernel_size, int img_height, int img_width) {
    img_blur_kernel << < this->gridSize, this->blockSize >> > (d_img_input, d_img_output, kernel_size, img_height, img_width);
}
